...
public class quicksort
{
    ...
    public static pokemon[] ordenacao(pokemon[] array, int low, int high) 
    {
        ...
        if (low < high) {
            ...
            ordenacao(array, low, pi - 1);
            ordenacao(array, pi + 1, high);
        }

        logTime.setEndTime();

        // Gravar o log no arquivo
        try (FileWriter fw = new FileWriter("844871_quicksort.txt")) 
        {
    ...
    private static int partition(pokemon[] array, int low, int high) 
    {
        int pivot = array[high].gen;  
        int i = (low - 1); 

        for (int j = low; j < high; j++) 
        {
            comp+=2;
            if (array[j].gen <= pivot) 
            {
                i++;
                mov++;
