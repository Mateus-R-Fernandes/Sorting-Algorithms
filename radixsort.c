...
int getMaxChar(Pokemon pokedex[], int tamanho, int pos)
{
    char max = pokedex[0].abilities[pos];
    for (int i = 1; i < tamanho; i++)
    {
        if (pokedex[i].abilities[pos] > max)
        {
            max = pokedex[i].abilities[pos];
        }
    }
    return (int)max;
}

// Função de ordenação Counting Sort para uma posição específica da string abilities
void countingSortAbilities(Pokemon pokedex[], int tamanho, int pos)
{
    Pokemon *output = malloc(tamanho * sizeof(Pokemon));
    int count[256] = {0};

    for (int i = 0; i < tamanho; i++)
    {
        int charPos = (int)pokedex[i].abilities[pos];
        count[charPos]++;
    }
    ...
    {
        int charPos = (int)pokedex[i].abilities[pos];
        output[count[charPos] - 1] = pokedex[i];
        count[charPos]--;
    }
...
// Função Radix Sort para ordenar Pokémons por abilities
void radixSortAbilities(Pokemon pokedex[], int tamanho)
{
    int maxLen = strlen(pokedex[0].abilities);

    for (int pos = maxLen - 1; pos >= 0; pos--)
    {
        countingSortAbilities(pokedex, tamanho, pos);
    }
}
...
int main() {
    ...
    start = clock();
    radixSortAbilites(pokedex, numPokemonsPokedex);
    end = clock();
